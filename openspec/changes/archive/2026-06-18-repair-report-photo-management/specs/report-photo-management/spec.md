# Delta for Report Photo Management
...
(Previously: The system allowed single-photo upload to a report and did not define optional multi-photo selection, previews, edit reconciliation, or error recovery.)
...
- WHEN the user uploads an image file larger than 5 MB
...
- GIVEN an authenticated user
- WHEN the user attempts to upload a photo to a report whose `readOnly` state is true
...
(Previously: The system displayed attached photos in report views and only described gallery behavior on the edit page.)
