 ...
 *
 * That block gives this screen a fixed height, and this screen is the one an operator meets on a short
 * viewport -- landscape above all -- where the branding plus the fields plus the button can exceed what
 * is available. The column therefore sits inside a scroll container, so a viewport too short to hold
 * its children scrolls to them instead of measuring a later child against shrinking space and squeezing
 * the password field or the submit button to nothing.
 *
 * The centring is kept rather than traded away for that scroll, with a minimum height equal to the
 * viewport on the content inside the container: short content still centres, tall content grows past the
 * viewport and scrolls. The web login centres this same branding-and-form block (`min-h-screen flex
 * items-center justify-center`), so matching it is part of matching the web at all.
 */
@Composable
fun LoginScreen(
